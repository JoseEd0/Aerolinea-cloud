...
import { createEquipment, type Equipment } from "@/api/microservicio3"
import { getPasajeros, type Pasajero } from "@/api/microservicio1"

export default function NuevoEquipajePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Éxito",
        description: "Equipaje registrado correctamente",
      })
      router.push("/equipajes")
    } catch (error) {
      toast({
        title: "Error",
        description: "No se pudo registrar el equipaje. Intente nuevamente.",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Nuevo Equipaje</CardTitle>
          <CardDescription>Complete los datos para registrar un nuevo equipaje</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? "Guardando..." : "Registrar Equipaje"}
            </Button>
          </CardFooter>
        </form>
      </Card>
